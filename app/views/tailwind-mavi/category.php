...
<div class="container mx-auto py-12">
            ...
            foreach ($categories as $categoryData):
                if ($categoryData['parent_id'] == 0): // Üst kategori kontrolü
                    ...
                    <a href="/category/<?=$categoryData['id']?>">
                        <div class="bg-white rounded-lg shadow-lg overflow-hidden transition-transform duration-300 ease-in-out transform hover:scale-105 hover:shadow-2xl">
                            <div class="p-6">
                                <h2 class="text-2xl font-bold"><?= htmlspecialchars($categoryData['name']) ?></h2>
                                <p class="text-blue-500 font-bold text-lg mt-4">Toplam Bulunan Ürün: <?=   $product->countByCategory($categoryData['id']);
                                    ?></p>
                                <p class="text-gray-500 font-bold text-[1.3vh] mt-4">Oluşturulma: <?= htmlspecialchars($categoryData['created_at']); ?></p>
                            </div>
                        </div>
                    </a>
                <?php
    ...
    <section class="py-12">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            if (!$hasChildCategory): ?>
                <p class="text-center text-red-300">Alt kategori bulunmamaktadır.</p>
            <?php endif; ?>
        </div>
    </section>
</div>
